...
config.SCHEMA = {
    ...
    { key = "yawRateKp",     group = "tank",   kind = "number", def = 2.0,  min = 0,   max = 5,   step = 0.1,
      help = "How hard the turn corrects the rate it is actually doing towards the rate it wants, in full differentials per top rate of error. This is the only term that can put the propellers into reverse to stop a turn. Zero flies on the ladder alone, which cannot stop.",
      symptom = "the hull coasts through the heading, or the differential chatters between the two extremes" },
    ...
    { key = "yawApproachMin", group = "tank",  kind = "number", def = 0.5,  min = 0.1, max = 20, step = 0.1,
      help = "Slowest turn the approach will ask for, deg/s, outside the padding. It has to stay well under what the hull can actually do, or a ship a degree or two off its heading is asked for most of the differential it owns.",
    ...
    { key = "yawFineBand",   group = "tank",   kind = "number", def = 0.25, min = 0.05, max = 10, step = 0.05,
      help = "Degrees of heading error inside which the turn is asked for nothing at all, because the ship has arrived. The brake still runs, so a nose swinging through this band is still stopped.",
      symptom = "it sits on the heading calling for RPM, or drifts off it and never corrects" },
    { key = "yawNearBand",   group = "tank",   kind = "number", def = 5.0,  min = 0.5, max = 45, step = 0.5,
      help = "Degrees of heading error inside which the push is the flat yawNearRpm instead of whatever the ladder and the approach price it at. The last few degrees of a turn are where a measured curve gives the worst answer.",
      symptom = "the last few degrees are taken at a speed the hull then has to undo" },
    { key = "yawNearRpm",    group = "tank",   kind = "int",    def = 64,   min = 0,   max = 256, step = 16,
      help = "The flat differential used inside yawNearBand, and the lowest the overshoot ceiling is ever allowed to fall to.",
      symptom = "the last few degrees crawl, or arrive too fast to stop" },
    { key = "yawMidRpm",     group = "tank",   kind = "int",    def = 128,  min = 0,   max = 256, step = 16,
      help = "Least differential a turn outside yawNearBand may be given. A floor, not a cap: a wide turn still gets everything the ladder asks for.",
      symptom = "a turn a few degrees wide of the heading takes for ever to close it" },
    { key = "yawOvershootEase", group = "tank", kind = "number", def = 0.6, min = 0.05, max = 1, step = 0.05,
      help = "How much of the push that sailed past the heading the next push is allowed. Under one, or the same overshoot happens again with the same force.",
      symptom = "it swings past the bearing, comes back past it, and keeps doing that" },
    { key = "yawOvershootRecover", group = "tank", kind = "number", def = 16, min = 0, max = 256, step = 4,
      help = "How fast the overshoot ceiling climbs back, RPM a second, while the hull stays on one side of the heading. A ship that has settled should not be flying on a limit it earned two swings ago.",
      symptom = "one bad swing early in a turn makes the whole rest of it sluggish" },
