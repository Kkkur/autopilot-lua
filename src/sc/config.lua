...
config.SCHEMA = {
    ...
    { key = "yawRateKp", group = "tank", kind = "number", def = 2.0, min = 0, max = 5, step = 0.1,
      help = "Maximum rate correction gain. The sampled response lowers it when holding this gain for a whole period could reverse the rate error. The ceiling limits how much an uncertain acceleration estimate can amplify feedback.",
      symptom = "an unmeasured hull chatters while trying to stop" },
    { key = "yawBrakeSafety", group = "tank",  kind = "number", def = 0.6,  min = 0.05, max = 1, step = 0.05,
      help = "How much of the measured yaw deceleration the approach trusts when it decides how early to slow down. Under one, because the number was measured once and the cargo moves.",
      symptom = "the hull sails past the heading, or crawls the last ninety degrees of every turn" },
    { key = "yawApproachMin", group = "tank",  kind = "number", def = 0.5,  min = 0.1, max = 20, step = 0.1,
      help = "Floor of the stopping profile ceiling, deg/s. Bounded by yawFineBand divided by the measured sample period; the sample limit can lower it further. This is not a minimum commanded rate.",
      symptom = "the turn stops a degree short and sits there, or calls for full RPM sitting on the heading" },
    { key = "yawStepFraction", group = "tank", kind = "number", def = 0.5, min = 0.05, max = 0.5, step = 0.05,
      help = "Fraction of the remaining heading error allowed per sample plus hull response time. At most one half keeps the sampled model overdamped with the default rate gain. Lower values leave more room for delay.",
      symptom = "the turn overshoots when the computer or propellers respond slowly" },
    { key = "yawFineBand",   group = "tank",   kind = "number", def = 0.25, min = 0.05, max = 10, step = 0.05,
      help = "Degrees of heading error allowed at rest. Thrust ends only when the current heading, predicted coasting heading and measured rate all fit. A fast swing still gets reverse thrust.",
      symptom = "it sits on the heading calling for RPM, or drifts off it and never corrects" },
    { key = "yawAccelAssumed", group = "tank", kind = "number", def = 1.0,  min = 0.1, max = 90, step = 0.1,
      help = "The yaw deceleration the approach assumes, deg/s/s, on a ship the yaw stage has not measured one for. Low on purpose: guessing low brakes early and guessing high sails past. Run the yaw stage, or type the real number into the yaw acceleration field, and this stops being used.",
      symptom = "an uncalibrated ship turns at full differential right up to the heading" },
    ...
    { key = "yawTrimThresh", group = "cruise", kind = "number", def = 0.5,  min = 0.05, max = 30, step = 0.1,
      help = "Degrees of heading error the running correction allows at rest. It ends thrust only when the error, the predicted coasting error and the measured rate all fit, exactly as the turn does.",
      symptom = "small corrections cost stress and buy nothing, or the heading drifts and is never picked up" },
    { key = "yawTrimRpm",    group = "cruise", kind = "int",    def = 128,  min = 0,   max = 256, step = 16,
      help = "Most differential the running correction may use. A correction is not a turn.",
      symptom = "a mid run correction turns into a full turn" },
    { key = "cruiseYawRateMax", group = "cruise", kind = "number", def = 6.0, min = 0.5, max = 90, step = 0.5,
      help = "Fastest yaw rate the running correction will ask for, deg/s. Well under yawRateMax, because a hull under forward thrust is steering with part of its authority and the bearing is moving under it.",
      symptom = "a mid run correction swings the nose about, or never catches a drifting heading" },
    { key = "fwdAccelAssumed", group = "cruise", kind = "number", def = 0.8, min = 0.05, max = 30, step = 0.05,
      help = "The forward acceleration assumed, m/s/s, on a ship nothing has measured one for. Low on purpose: a low guess lengthens the modelled response and asks for less per sample. Fill in the forward response fields, and this stops being used.",
      symptom = "an unmeasured ship surges at the start of every leg" },
    { key = "fwdBrakeSafety", group = "cruise", kind = "number", def = 0.6, min = 0.05, max = 1, step = 0.05,
      help = "How much of the forward acceleration the response model trusts when it works out the hull's time constant. Under one, because it lengthens the modelled response and a longer response asks for less.",
      symptom = "the speed hunts about the wanted figure, or takes an age to reach it" },
    { key = "fwdRateKp",     group = "cruise", kind = "number", def = 2.0, min = 0,   max = 5,  step = 0.1,
      help = "Ceiling on the speed response gain. The sampled response lowers it when holding a command for a whole period could reverse the speed error. The ceiling limits how much an uncertain acceleration estimate can amplify feedback.",
      symptom = "an unmeasured hull surges back and forth about the wanted speed" },
    { key = "fwdStepFraction", group = "cruise", kind = "number", def = 0.5, min = 0.05, max = 1, step = 0.05,
      help = "Fraction of the remaining speed error the reference may spend per sample plus hull response time. At most one half keeps the sampled model overdamped at the default gain.",
      symptom = "the speed overshoots when the computer or propellers respond slowly" },
    { key = "fwdAssumeResponse", group = "cruise", kind = "bool", def = false,
      help = "Ignore any measured forward response and use the assumed figures. The honest setting on a ship whose response fields were typed in rather than measured.",
      symptom = "a response number somebody guessed is being flown as though it were measured" },
    ...
    { key = "brakeSlew",     group = "brake",  kind = "int",    def = 16,   min = 1,   max = 256, step = 4,
      help = "How fast reverse comes on, RPM per control update. Suddenness is what tips the hull. Used when slewTimed is off.",
      symptom = "the nose dips hard the moment braking starts" },
    { key = "brakeSlewRate", group = "brake",  kind = "number", def = 64.0, min = 4,   max = 2048, step = 8,
      help = "The same, in RPM per second, used when slewTimed is on. The default is brakeSlew over a reference period of 0.25 seconds, which is what the old per update figure meant on a loop running at that rate.",
      symptom = "reverse comes on at a different speed once the loop period changes" },
    { key = "brakeAccelAssumed", group = "brake", kind = "number", def = 0.8, min = 0.05, max = 30, step = 0.05,
      help = "The deceleration assumed, m/s/s, in a direction the brake stage has not measured. Low on purpose: guessing low starts the stop early. The brake stage measures stopping from forward motion only, so stopping from backward motion is always this figure.",
      symptom = "a stop from a direction nothing measured begins far too late, or absurdly early" },
    { key = "brakeAssume",   group = "brake",  kind = "bool",   def = true,
      help = "Plan stops on brakeAccelAssumed where there is no measured ladder. Off refuses to move in a direction with no braking data at all, which is honest and grounds an uncalibrated ship.",
      symptom = "you would rather it refused the leg than flew one on an assumed stopping distance" },
    { key = "actuatorLag",   group = "brake",  kind = "number", def = 0.5,  min = 0,   max = 10,  step = 0.1,
      help = "Seconds between a command being decided and the propellers actually turning at it, counting the slew, the radio and the relay. The stopping distance is planned with the ship still travelling through it.",
      symptom = "stops begin on time and still end past the point" },
    { key = "brakeYaw",      group = "brake",  kind = "bool",   def = true,
      help = "Keep correcting the heading during a stop. The correction is worked out fresh every update; it is never the one cruise last used.",
      symptom = "the nose wanders through every stop, or a stop fights itself for steering" },
    { key = "brakeYawRecruit", group = "brake", kind = "bool",  def = false,
      help = "Let the need to steer bring the turbines out of a stop that is running on the main alone. Off keeps the graduated stop the brake stage measured and says the heading was left alone.",
      symptom = "the ship loses its nose during a gentle stop, or a gentle stop turns into a hard one to hold a heading" },
    ...
    { key = "arriveSpeed",   group = "arrival", kind = "number", def = 0.3,  min = 0.01, max = 10, step = 0.05,
      help = "Speed along the hull, m/s, low enough to count as stopped. Part of the terminal state: the position, this, the drift and the predicted coast must all fit before thrust is frozen at zero.",
      symptom = "it calls itself stopped while still sliding, or never admits it has stopped" },
    { key = "arriveDrift",   group = "arrival", kind = "number", def = 0.5,  min = 0.01, max = 10, step = 0.05,
      help = "Speed through the air, m/s, low enough to count as stopped. Larger than arriveSpeed because it counts the sideways and vertical motion a hull that cannot strafe has no way to null.",
      symptom = "a ship drifting sideways over the point is filed as having arrived" },
    ...
    { key = "rpmSlew",       group = "mix",    kind = "int",    def = 64,   min = 4,   max = 256, step = 8,
      help = "Most RPM one line may change per control update. Softens the shove. Used when slewTimed is off.",
      symptom = "demands arrive as a kick rather than a push" },
    { key = "rpmSlewRate",   group = "mix",    kind = "number", def = 256.0, min = 8,  max = 4096, step = 16,
      help = "The same, in RPM per second, used when slewTimed is on. The default is rpmSlew over a reference period of 0.25 seconds, which is what the old per update figure meant on a loop running at that rate.",
      symptom = "thrust comes on at a different speed once the loop period changes" },
    { key = "slewTimed",     group = "mix",    kind = "bool",   def = false,
      help = "Measure the slew in RPM per second rather than per control update. Off is the per update limit the turn was validated with. The fractional remainder is carried either way, so repeated short periods still move a line.",
      symptom = "a slow server makes every demand arrive as a kick, because a slow update is still one update" },
    { key = "yawHeadroom",   group = "mix",    kind = "bool",   def = true,
      help = "Hold back turbine thrust while running so there is range left on both sides for the heading correction. A saturated side cannot be pushed further, and clipping one side alone changes the common thrust as well as losing the correction. Stopping never gives up braking for this.",
      symptom = "the heading wanders at full cruise thrust and the correction seems to do nothing" },

    -- == GAINS ===============================================
    -- What is left of the speed loop once the measured ladder and the sampled
    -- response model have had their say. There is no second high gain loop on
    -- top of them: this is a slow trim for what the ladder did not know about.
    { key = "fwdTrimKi",     group = "gains",  kind = "number", def = 0.15, min = 0,   max = 5,   step = 0.05,
      help = "How fast the speed trim winds on, in m/s of reference per m/s of error per second. It covers a headwind or a heavier hull than the day the ladder was measured, and nothing faster.",
      symptom = "it runs steadily under or over the wanted speed" },
    { key = "fwdTrimMax",    group = "gains",  kind = "number", def = 2.0,  min = 0,   max = 20,  step = 0.5,
      help = "Clamp on the speed trim, m/s. The trim is frozen while the propellers are saturated and reset on a new leg, a reversal, a stop and any phase change, so it cannot leak between legs.",
      symptom = "a long slow leg ends with the reference wound up to nothing useful" },
    ...
    { key = "calNorthYaw",   group = "cal",    kind = "number", def = 180,  min = -180, max = 180, step = 5,
      help = "The yaw north sits at, which in Minecraft is 180. Only used when the dimension has no magnetic north to report, and the align stage asks before flying on it instead. This is not how a ship whose front is the other end from its hull is corrected: the front offset is, and the align stage asks for that outright.",
      symptom = "a dimension with no magnetic north walks its compass rose a fixed amount off every point" },
    ...
    { key = "calAutoSeconds", group = "cal",   kind = "number", def = 30.0, min = 5,   max = 300, step = 5,
      help = "Longest an unattended rung runs before it is kept anyway.",
      symptom = "an unattended rung sits for ever on a reading that never settles" },
    { key = "calAutoMin",    group = "cal",    kind = "number", def = 6.0,  min = 1,   max = 120, step = 1,
      help = "Shortest an unattended rung runs, however steady it looks.",
      symptom = "an unattended rung keeps a zero the hull had not started answering yet" },
